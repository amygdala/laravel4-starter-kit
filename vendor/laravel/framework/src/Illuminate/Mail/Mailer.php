...
class Mailer {
	...
	public function send($view, array $data, $callback)
	{
		...
		if(isset($_SERVER['SERVER_SOFTWARE']) && strpos($_SERVER['SERVER_SOFTWARE'],'Google App Engine') !== false) {
			syslog(LOG_DEBUG, "Sending mail via the app engine mail api");
			try {
				$emails = implode(', ', array_keys((array) $message->getTo()));
				$subj = $message->getSubject();
				$body = $message->getBody();
				syslog(LOG_DEBUG, "to: " . print_r($emails, true));
				syslog(LOG_DEBUG, "subj: " . print_r($subj, true));
				syslog(LOG_DEBUG, "body: " . print_r($body, true));
				$mail_options = [
				    "sender" => "admin@aju-laravel.appspotmail.com",
				    "to" => $emails,
				    "subject" => $subj,
				    // "textBody" => "this is really a test"
				    "htmlBody" => $body
				];
