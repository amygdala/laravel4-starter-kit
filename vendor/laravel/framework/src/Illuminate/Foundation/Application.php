...
class Application extends Container implements HttpKernelInterface, ResponsePreparerInterface {
	...
	public function bindInstallPaths(array $paths)
	{
		...
		{
			// syslog(LOG_WARNING, "path: $value for $key");
			if (realpath($value)) {
				syslog(LOG_WARNING, "realpath");
				$this->instance("path.{$key}", realpath($value));
			}
			elseif (file_exists($value)) {
				// syslog(LOG_WARNING, "file exists: " . $value);
				$this->instance("path.{$key}", $value);
			}
			else {
				// syslog(LOG_WARNING, "file does not exist: $value");
	...
	{
		// aju temp
		syslog(LOG_DEBUG, "request: " . $this['request']);
